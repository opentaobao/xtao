...
function buildConfig(configPath){
	...
	for(var k in groupsui){
		allOptionsUI = allOptionsUI.concat(groupsui[k]);
	}
...
function _buildGroups(groups){
	if(u.isEmptyObject(groups)){
		return {"default": []};
	}

	var groupsui = {};
	for(var k in groups){
		groupsui[k] = [ui.generateGroup({label: groups[k].label})];
	};
	groupsui['default'] = [];
	return groupsui;
};
