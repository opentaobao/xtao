 ...
 * }
 */
, generateSelectOption: function(cfg){
		var controls = [
...
, generateListOption: function(cfg){
		...
		cfg.options.forEach(function(option){
			if(cfg.default === option.key){
				entry = '<input data-event="click" class="op-trigger" type="radio" name="' + cfg.key + '-radio" value="' + option.key + '" checked />' + option.label;
			}else{
				entry = '<input data-event="click" class="op-trigger" type="radio" name="' + cfg.key + '-radio" value="' + option.key + '" />' + option.label;
			}
			options.push(listTpl.replace(/{{entry}}/, entry));
		});

		var html = this.generateOption()
								.replace(/{{controls}}/g, options.join(u.getEOL()))
								.replace(/{{id}}/g, id++)
								.replace(/{{label}}/g, cfg.label)
								.replace(/{{help}}/g, this.generateHelp(cfg.help))
								.replace(/{{data}}/g, cfg.key + "/list");
